...
//    println(plusOne) // This is a new list
    ...
    //println(twice(List()))
    //println(drunkWords(List("Hey","you,","how","are","you","doing?")))
    //val startsWithS = (s: String) => s.startsWith("s") // lambda expression
    //println(myForAll(List("sbc", "sef"), startsWithS))
    //println(lastElem(List())) // None
    //println((lastElem(List.range(0,3))))// Some(2) (range has exclusive ceiling))
    //println(append(List(), List())) 	// List(1,3,5,2,4)
    val nrs = List.range(0,11) // List(0,1,2,3,...,10)
    println(myFilter(nrs, (i: Int) => i % 2 == 0))// List(0,4,8)
  }
...
//    def reverse(xs: String): String =
    ...
    case Nil => List()
    case xs :: t => drunkWords(t) ::: List(xs.reverse)
  }
   ...
   */
  // TODO implement using pattern matching
  def append[A](xs: List[A], ys: List[A]): List[A] = xs :::ys
  ...
  def helper[A](xs: List[A], f: A => Boolean, index: Int ): List[A] = xs match {
    case Nil => List()
    case x :: t=> if(f(x)) {
      if (index % 2 == 1) x :: helper(t,f,index+1) else helper(t,f,index+1)
    } else helper(t,f,index)
  }
